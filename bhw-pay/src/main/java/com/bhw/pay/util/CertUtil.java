...
import static com.bhw.pay.common.SDKConstants.UNIONPAY_CNNAME;
...
public class CertUtil {
    ...
    public static void init() {
        try {
            ...
            logger.info("init失败。（如果是用对称密钥签名的可无视此异常。）", e);
        }
    }
    ...
    private static void initSignCert() {
        ...
        if (SDKConfig.getConfig().getSignCertPath() == null
            ...
            logger.info("WARN: " + SDKConfig.SDK_SIGNCERT_PATH + "或" + SDKConfig.SDK_SIGNCERT_PWD
    ...
    public static X509Certificate getMiddleCert() {
        if (null == middleCert) {
            ...
            if (!SDKUtil.isEmpty(path)) {
                ...
                logger.info(SDKConfig.SDK_MIDDLECERT_PATH + " not set in " + SDKConfig.FILE_NAME);
                return null;
            }
        }
        return middleCert;
    ...
    public static X509Certificate getRootCert() {
        if (null == rootCert) {
            ...
            if (!SDKUtil.isEmpty(path)) {
                ...
                logger.info(SDKConfig.SDK_ROOTCERT_PATH + " not set in " + SDKConfig.FILE_NAME);
                return null;
            }
        }
        return rootCert;
