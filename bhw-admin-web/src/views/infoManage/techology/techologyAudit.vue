        ...
        <div class="bgColorF" style="padding: 0 25px;">
            ...
            <el-form :inline="true" :model="formInline"  class="demo-form-inline" style="margin: 33px 40px;">
                ...
                <el-form-item label="技术名称：" prop="name">
                    <el-input placeholder="选填" v-model="formInline.techName"></el-input>
                </el-form-item>
                <el-form-item label="行业类型：" prop="name">
                    <el-select v-model="formInline.intellectualType" placeholder="请选择 ">
                        <el-option
...
<script>
    ...
    import {getBussinessType,getInfoList,infoAudit} from "../../../api/infoManage/techology";
    ...
    export default {
        ...
        methods:{
            ...
            handleArgee(row){
                ...
                var req={type:1,idList:idList.join(',')}
                infoAudit(req).then(response => {
                    ...
                    if(row){
                        var msg={
                            userId:row.id,
                            productId:null,
                            content:'尊敬的用户,您您发布的产品:'+row.techName+'已通过！',
                            msgType:3
                        }
                    ...
                    var par = {
                        ...
                        content:undefined==row?'对技术供需下的产品进行了批量审核为通过':'对技术供需下的产品('+row.serverName+')进行了审核为通过'
                    };
            ...
            handleDisAgree(row){
                ...
                var req={type:2,idList:idList.join(',')}
                this.$prompt('请输入驳回原因', '提示', {
                    ...
                    infoAudit(req).then(response => {
                        ...
                        if(row){
                            var msg={
                                userId:row.userId,
                                productId:row.id,//'尊敬的用户,您发布的‘+row.techName+’被驳回。原因如下:'+value+'！',
                                content:'尊敬的用户,您发布的:'+row.techName+'被驳回。原因如下:'+value+'!',
                                msgType:3
                            }
                            saveMsg(msg).then(response=>{});
                        }
                        var par = {
                            operateName:this.$store.getters.userName,
                            operateId:this.$store.getters.backUserId,
                            content:undefined==row?'对技术供需下的产品进行了批量审核为驳回':'对技术供需下的产品('+row.serverName+')进行了审核为驳回'
                        };
                        saveBackOperate(par).then();
