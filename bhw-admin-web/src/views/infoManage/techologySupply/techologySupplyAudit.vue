...
<script>
    ...
    import {getInfoList,infoAudit} from "../../../api/infoManage/techSupply";
            ...
            handleArgee(row) {
                ...
                infoAudit(req).then(response => {
                    ...
                    var msg={
                        userId:row.id,
                        productId:null,
                        content:'尊敬的用户,您您发布的产品:'+row.serverName+'已通过！',
                        msgType:3
                    }
                    ...
                    var par = {
                        ...
                        content:undefined==row?'对技术合同下的产品进行了批量审核为通过':'对技术合同下的产品('+row.serverName+')进行了审核为通过'
                    };
                ...
                this.$prompt('请输入驳回原因', '提示', {
                    ...
                    infoAudit(req).then(response => {
                        ...
                        var par = {
                            operateName:this.$store.getters.userName,
                            operateId:this.$store.getters.backUserId,
                            content:undefined==row?'对技术合同下的产品进行了批量审核为驳回':'对技术合同下的产品('+row.serverName+')进行了审核为驳回'
                        };
                        saveBackOperate(par).then();
                    })
                }).catch(() => {
