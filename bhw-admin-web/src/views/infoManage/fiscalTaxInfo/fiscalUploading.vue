<template>
    <el-dialog title="财税服务"
            :visible.sync="fiscalDialogVisible"
            width="60%" class="serviceDialog">
    <div class="Uploading" style="padding-left: 80px;">
        <el-form ref="form" label-width="140px" label-position="right" :model="productInfo">
            ...
            <el-form-item label="联系人：" prop="name">
                ...
                <div style="width:142px;float: left;" class="danwei">
                    <el-input placeholder="必填" style="width: 202px;" v-model="productInfo.contactUnit">
                    </el-input>
                </div>
            </el-form-item>
            ...
            <el-form-item style="margin-top: 45px;">
                <el-button  class="btnColorOrg btnSave">取消</el-button>
                <el-button class="btnColorBlue btnUp" type="primary" @click="onSubmit">上传发布</el-button>
            </el-form-item>
        </el-form>
    </div>
    </el-dialog>
...
<script>
    ...
    import {getBussinessType,upFiscalTaxInfo,saveReleaseInfo} from "../../../api/infoManage/fiscalTaxInfo";
    import {qryProvinceList,qryCityList} from "../../../api/admin/index";
    export default {
        name: "fiscalUploading",
        data() {
            return {
                fiscalDialogVisible:false,
                radio: '1',
                options: [
                    {id: "", name: ""},
                ],
            ...
            onSubmit() {
                if (this.productInfo.checkType==''){
                    ...
                    if(this.productInfo.id){
                        ...
                        upFiscalTaxInfo(this.productInfo).then(response => {
                            this.$message.info('上传成功!');
                            var par = {
                                operateName:this.$store.getters.userName,
                                operateId:this.$store.getters.backUserId,
                                content:'编辑了财税服务产品('+this.productInfo.serverName+')'
                            };
                            saveBackOperate(par).then();
                            this.cleanData();
                            this.fiscalDialogVisible = false;
                        ...
                        saveReleaseInfo(this.productInfo).then(response => {
                            ...
                            var par = {
                                operateName:this.$store.getters.userName,
                                operateId:this.$store.getters.backUserId,
                                content:'发布了财税服务产品('+this.productInfo.serverName+')'
                            };
                            saveBackOperate(par).then();
                            this.cleanData();
                            this.fiscalDialogVisible = false;
...
<style scoped>
    ...
    .Uploading /deep/ .el-input--suffix .el-input__inner {
        width: 308px;
        height: 46px;
    ...
    .lianxi /deep/ .el-input {
        width: 142px;
        height: 46px;
        background: rgba(255, 255, 255, 1);
    }

    .danwei /deep/ .el-input {
        width: 392px;
        height: 46px;
        background: rgba(255, 255, 255, 1);
    }

    .guanjian /deep/ .el-input {
        width: 308px;
        height: 46px;
        background: rgba(255, 255, 255, 1);
    }

    /deep/ .quxiao.el-button.el-button--default span {
        font-size: 14px;
