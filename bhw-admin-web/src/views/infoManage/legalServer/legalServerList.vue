<template>
    <div class="listWarp" style="padding: 22px 16px 0;">
        ...
        <LegalUploading ref="LegalUploadingRef"></LegalUploading>
    </div>
</template>

<script>
    import {saveBackOperate} from "../../../api/admin/index";
    import {getInfoList,infoAudit,getLegalServerInfo,serverTakeOff,upLegalServerInfo,delLegalServerInfo} from "../../../api/infoManage/legalServer";
    import LegalUploading from "../legalServer/LegalUploading";
    export default {
        name: "legalServerList",
        comments: { LegalUploading },
        data(){
            return {
                formInline:{
                    checkType:"",
        ...
        methods: {
            ...
            addLegalBtn(){
                this.$nextTick(function() {
                    this.$refs.LegalUploadingRef.legalDialogVisible = true;
                });
            },
            handleUp(row){
                getLegalServerInfo({id:row.id}).then(response=>{
                    this.$nextTick(function() {
                        this.$refs.LegalUploadingRef.productInfo = response.data;
                        this.$refs.LegalUploadingRef.productInfo.cityId=null;
                        this.$refs.LegalUploadingRef.legalDialogVisible = true;
                    });
                });
            },
            upHandleDel(row){
                upLegalServerInfo({id:row.id,status:2}).then(response=>{
                    this.$message({
                        type: 'success',
                        message: '操作成功!'
                    });
                ...
                this.$confirm('此操作将下架选择信息, 是否继续?', '提示', {
                    ...
                    serverTakeOff({idList:idList.join(',')}).then(response=>{
                        ...
                        var par = {
                            ...
                            content:undefined!=row?'对法律服务下的产品('+row.serverName+')进行了下架':'对法律服务下的产品进行了批量下架'
                        };
