...
<script>
    ...
    import {getInfoList,infoAudit,getLegalServerInfo} from "../../../api/infoManage/legalServer";
            ...
            handleArgee(row) {
                ...
                infoAudit(req).then(response => {
                    ...
                    var par = {
                        ...
                        content:undefined==row?'对法律服务下的产品进行了批量审核为通过':'对法律服务下的产品('+row.serverName+')进行了审核为通过'
                    };
                })
            },
            handleDisAgree(row) {
                ...
                this.$prompt('请输入驳回原因', '提示', {
                    ...
                    infoAudit(req).then(response => {
                        ...
                        var par = {
                            operateName:this.$store.getters.userName,
                            operateId:this.$store.getters.backUserId,
                            content:undefined==row?'对法律服务下的产品进行了批量审核为驳回':'对法律服务下的产品('+row.serverName+')进行了审核为驳回'
                        };
