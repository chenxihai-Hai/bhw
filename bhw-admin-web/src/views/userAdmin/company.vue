        ...
        <div class="bgColorF" style="padding: 38px 25px;">
            <el-form :inline="true" :model="formSearch" class="demo-form-inline">
                ...
                <el-form-item label="邮箱" :label-width="formLabelWidth">
                    <el-input placeholder="请输入" v-model="formSearch.emall" class="allIpt"></el-input>
                </el-form-item>
            ...
            <div style="margin:20px 20px 0;">
                ...
                <el-table :data="tableData" border style="width: 100%" class="allTable" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="用户类型">
                        <template slot-scope="scope">{{scope.row.type==1?'商家用户':'普通企业用户'}}</template>
                    </el-table-column>
                    ...
                    <el-table-column label="头像">
                        <template slot-scope="scope">
                            <el-image style="width: 129px; height: 78px" :src="scope.row.photo" alt=""/>
                        </template>
                    </el-table-column>
            ...
            <el-form :model="editForm" class="demo-form">
                ...
                <el-form-item label="头像" :label-width="formLabelWidth" class="uploadBox">
                    <el-upload
                            ...
                            :file-list="fileList"
                            list-type="picture">
                        <el-button size="small" type="primary" class="btnColorOrg">上传头像</el-button>
                    </el-upload>
                </el-form-item>
                <el-form-item label="性别" :label-width="formLabelWidth">
    ...
    export default {
        ...
        data() {
            return {
                ...
                userTypeData:[{typeId:2,typeName:'普通用户'},{typeId:1,typeName:'商家用户'}],
                sexList:[{sexId:1,sexName:'男'},{sexId:2,sexName:'女'}],
                formSearch:{userId:'',userName:null,phone:null,email:null,statusId:null,userType:null,sex:null,type:2,curPage: 1, pageSize:10},
                //tableData: [{id:"",userName:"",type:"",status:"",photo:"https://fuss10.elemecdn.com/3/63/4e7f3a15429bfda99bce42a18cdd1jpeg.jpeg?imageMogr2/thumbnail/360x360/format/webp/quality/100",sex:"",email:"",phone:"",name:""}],
                tableData:[],
                editForm:{userName:null,phoneNo:null,emall:null,photo:null,sex:null,id:''},
                ...
                this.$confirm('确认删除该信息', '确认', {
                    ...
                    deleteUserList(this.deleteForm).then(response => {
                        var par = {
                            ...
                            content:'在用户管理下删除了企业用户'+row.userName
                        };
                ...
                if (this.editForm.userName==null) {
                    ...
                    editUser(this.editForm).then(response => {
                        var par = {
                            operateName:this.$store.getters.userName,
                            operateId:this.$store.getters.backUserId,
                            content:'在用户管理下对企业用户'+this.editForm.userName+'进行了编辑'
                        };
                        saveBackOperate(par).then();
                        if (response.data) {
