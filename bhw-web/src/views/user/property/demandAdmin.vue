<template>
    <div class="demandAdmin" style="padding: 0 10px 80px 20px;">
        <div style="padding-left: 50px;">
            <el-form ref="form" >
                <el-form-item label="供需选择：" prop="name">
                    ...
                    <span style="margin:0 24px 0 60px;">业务类型</span>
        ...
        <div class="showOrHide" style="margin: 92px auto 25px;"><span @click="shou" :class="toggle==false?'hideS':''" style="position: relative;">{{toggle==true?'收起':'展开'}}</span></div>
        <div v-if="toggle">
        ...
        <template v-for="temp in xuqiu">
            <div class="gongxu" :key="temp.id">
                <el-row>
                    <el-col :span="12"><el-checkbox class="check" v-model="temp.check"></el-checkbox><span>选择</span></el-col>
                    <el-col :span="12" style="text-align: right;" class="img-gx"><img src="../../../images/property/xu.png" class="xu"/></el-col>
                </el-row>
                <el-row type="flex" justify="space-between" align="middle" class="tInfo" style="margin: 14px 0 18px;">
                ...
                <el-row class="tInfo mark">
                    ...
                    <el-col class="jianjie" style="margin-top: 13px;">
                        <span>简介：</span><span>{{temp.serverRemark}}</span>
                    </el-col>
                </el-row>
                <el-row type="flex" align="middle" class="tInfo" style="margin: 55px 0 0;">
                    <el-col :span="8"><span style="color:#024ECC;">需求类型 | {{temp.businessName}} </span></el-col>
                    <el-col :span="8" class="area-col"><img src="../../../images/property/dizhi.png" class="dingwei"><span>定位：{{temp.provinceName}}&nbsp;{{temp.cityName}}</span></el-col>
                    <!--<el-col :span="8">
                        <el-button class="btnColorOrg" @click="upFiscalTaxInfo(temp.id,1)">
                            已承接
                        </el-button>
                        <el-button class="btnColorBlue" @click="upFiscalTaxInfo(temp.id,2)">
                            已解决
                        </el-button>
                    </el-col>
                    <el-col :span="8" style="text-align: right;">
                        <el-button class="btnColorOrg  btnAskMe">谁联系了我</el-button>
                    </el-col>-->
...
<script>
    import {getBussinessType, getServerListPage, serverTakeOff,upFiscalTaxInfo,delFiscalTaxInfo} from "../../../api/user/fiscalTax";
    ...
    export default {
        ...
        methods: {
            ...
            excelBtn(){
                getServerListPage({pageSize:1000000}).then(response=>{
                    ...
                    require.ensure([], () => {
                        ...
                        export_json_to_excel(tHeader, data, "财税服务明细");
                    });
                });
            },
            ...
            upFiscalTaxInfo(id,st){
                upFiscalTaxInfo({id:id,result:st}).then(response=>{
                    this.$message({
                        type: 'success',
                        message: '操作成功!'
                    });
            ...
            onSubmit() {
                if (this.Time.length==2){
                    this.sousuo.startTime=this.Time[0].toString().substring(0,9);
                    this.sousuo.endTime=this.Time[1].toString().substring(0,9);
                }
            ...
            getBussinessType({type: 14}).then(response => {
...
<style scoped>
    ...
    .btnAskMe {width: 78px;height: 34px;font-size: 12px; padding: 0;border: none;}
    ...
    .gongxu {background: rgba(255, 255, 255, 1);padding: 20px 20px 37px;margin-bottom: 45px; box-shadow: 0px 1px 17px 1px rgba(0, 0, 0, 0.14);position: relative;}
    .img-gx{position: absolute;top:-20px;right: 0;}
