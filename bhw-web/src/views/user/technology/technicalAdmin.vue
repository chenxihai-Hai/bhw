<template>
    <div class="technicalAdmin" style="padding: 0 12px 80px;">
        <div>
            <el-form ref="form" >
                <el-form-item label="技术编号：" prop="name">
                    <div style="float: left; margin-right: 38px" class="bianhao">
                        <el-input placeholder="选填" v-model="sousuo.techCode">
                        </el-input>
                    </div>
                    <span style="margin-right: 16px">提交时间</span>
...
<!--                    </el-form-item>-->
                ...
                <el-form-item label="技术名称：" prop="name">
                    <div style="float: left; margin-right: 38px" class="bianhao">
                        <el-input placeholder="选填" v-model="sousuo.techName">
                        </el-input>
                    </div>
                    <span style="margin-right: 16px">所属类型</span>
                    <el-select v-model="sousuo.businessType" placeholder="请选择 ">
                        <el-option
                ...
                <el-radio v-model="sousuo.checkType" label=1>供类（我要卖出）</el-radio>
                <el-radio v-model="sousuo.checkType" label=2>需类(我要买)</el-radio>
        ...
        <div class="showOrHide"><span @click="shou" :class="toggle==false?'hideS':''" style="position: relative;">{{toggle==true?'收起':'展开'}}</span></div>
        <div v-if="toggle">
        ...
        <template v-for="temp in xuqiu">
            <div class="gongxu" :key="temp.id">
                ...
                <el-row type="flex" align="middle" class="tInfo" style="margin: 61px 0 0;">
                    <el-col :span="8">
                        <span style="color: #024ECC;">需求类型 | {{temp.businessName}} </span>
                    </el-col>
                    ...
                    <el-col :span="8"  v-if="temp.status!=1">
                        <el-button class="btnColorOrg" @click="upTechologyInfo(temp.id,1)">
                            已承接
                        </el-button>
                        <el-button class="btnColorBlue" @click="upTechologyInfo(temp.id,2)">
                            已解决
                        </el-button>
                    </el-col>
                    <el-col v-if="temp.status==1">
...
<script>
    import {getBussinessType, getServerListPage, serverTakeOff,upTechologyInfo,delTechologyInfo} from "../../../api/user/techology";
    ...
    export default {
        ...
        methods: {
            ...
            excelBtn(){
                getServerListPage({pageSize:1000000}).then(response=>{
                    ...
                    require.ensure([], () => {
                        ...
                        export_json_to_excel(tHeader, data, "技术供需明细");
                    });
                });
            },
            ...
            upTechologyInfo(id,st){
                upTechologyInfo({id:id,result:st}).then(response=>{
                    this.$message({
                        type: 'success',
                        message: '操作成功!'
                    });
...
<style scoped>
    ...
    .gongxu {width: 947px;background: rgba(255, 255, 255, 1);padding: 20px 20px 50px;margin-bottom: 50px; box-shadow: 0px 1px 17px 1px rgba(0, 0, 0, 0.14);}
